...
'''
###########
# K值聚类 # 一个迭代算法，最终实现将数据分成多个组的形式
###########

#原理：首先选取两个任意点作为初始重心，分别计算其他点到两点的距离. 离哪个重心距离更小，
###### 该点就标记为属于哪个重心. 然后重新计算所有同类点的重心，继续以计算出的两个重心进
###### 行迭代，更新所有的点标记，直至两个重心点稳定下来，此时这些点到它们相应重心的距离
###### 之和最小（也可以通过设置迭代次数或阈值来提前终止迭代）。

#####################
# OpenCv中的K值聚类 # cv2.kmeans()
#####################

#输入参数：
#samples: np.float32类型的数据,每个特征应该放在一列

#nclusters(K):聚类的最终数目

#criteria：终止迭代条件，它应该是一个含有3个成员的元组（type, max_iter, epsilon)
##type取值：
## cv2.TERM_CRITERIA_EPS 只有满足精确度epsilon时才停止迭代
## cv2.TERM_CRITERIA_MAX_ITER 当迭代次数超过阈值时停止迭代
## cv2.TERM_CRITERIA_EPS+cv2.TERM_CRITERIA_MAX_ITER 满足以上任何一个条件是停止
##max_iter表示最大迭代次数
##epsilon表示精确度阈值

#attermpts:使用不同的起始标记来执行算法的次数,算法会返回紧密度最好的标记,紧密度也会作
###########输出被返回

#flags:用来设置如何选择起始重心,通常我们有两个选择：cv2.KMEANS_PP_CENTERS 和
...
#np.vstack()将（x,y)变为列向量,不同于np.hstack()将(x,y)的列相连.
z = np.vstack((x,y))
...
'''
#颜色量化
#颜色量化是减少图片中颜色数目的一个过程.为什么要减少图片中的颜色呢？减少内存消耗！有些
#设备的资源有限，只能显示很少的颜色。在这种情况下就需要进行颜色量化。我们使用 K 值聚类
#的方法来进行颜色量化。
#现在有 3 个特征：R，G，B。所以我们需要把图片数据变形成 Mx3（M 是图片中像素点的数目）的
#向量。聚类完成后，我们用聚类中心值替换与其同组的像素值，这样结果图片就只含有指定数目
#的颜色了。
'''
img = cv2.imread('messi.jpg')
Z = img.reshape((-1,3))
...
compactness,label,centers = cv2.kmeans(Z,8,None,criteria,10,flags)

centers = np.uint8(centers)
#label.flatten()将label降为一维
#numpy中以label数组为索引,从centers数组中索引出需要的元素,例如label[1] = 6,那么在
#label[1]位置上应该填入centers数组中索引6的元素,所以最终的输出数组的大小应该为
#label的列数 x centers 的列数
...
############################## 以 Haar 特征分类器为基础的对象检测技术是一种非常有效的对
# 使用Haar分类器进行面部检测 # 检测技术它是基于机器学习的，通过使用大量的正负样本图像训
############################## 练得到一个 cascade_function，最后再用它来做对象检测。

#OpenCv中的Haar级联检测
#OpenCv已经包含了很多已经训练好的分类器,其中包括:面部,眼睛,微笑等，这些XML文件保存在
...
#CascadeClassifier.detectMultiScale(image,scaleFactor,minNeighbors,flags,minSize,maxSize)
#scaleFactor：缩放因子,指定每个图像的大小的缩放比例
#minNeighbors：每个候选矩形应该保留多少个相邻矩形
#minSize：最小识别大小,小于该大小的矩形就忽略
#maxSize: 最大识别大小,超过该大小的矩形就忽略
#返回的是识别对象的外接矩形
